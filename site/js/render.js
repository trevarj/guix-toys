...
export function card(type, row) {
  ...
  switch (type) {
    case "packages":
      return `<article class="item" data-href="${href}">
  ...
  switch (type) {
    case "packages": {
      ...
      return `<article class="detail">
        ...
        ${detailRow("origin", origin?.uri ? `<code>${esc(Array.isArray(origin.uri) ? origin.uri[0] : origin.uri)}</code>` : "")}
    ...
    case "symbols":
      return `<article class="detail">
        <header class="d-head"><h1>${esc(row.name)}</h1>${channelTag(row.channel)}</header>
        ${row.signature ? `<pre class="d-code">${esc(row.signature)}</pre>` : ""}
    ...
    case "channels":
      return `<article class="detail">
        ...
        ${row.subscriptionSnippet ? `
          <div class="d-row"><span class="d-label">subscribe</span></div>
          <pre class="d-code" data-copy>${esc(row.subscriptionSnippet)}</pre>
          <button class="copy-btn" data-copy-btn>copy snippet</button>` : ""}
      </article>`;
  }
}
...
export function groupSection(type, label, rows, count, state) {
  ...
  return `<section class="group">
    <h2 class="group-title">${esc(label)} <span class="group-count">${count}</span>
      ${count > rows.length ? `<a class="group-more" href="${moreHash}">all ${count} →</a>` : ""}
    </h2>
